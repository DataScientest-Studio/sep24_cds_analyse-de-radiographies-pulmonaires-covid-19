...
DESCRIPTIONS = {
    ...
    'Isolation Forest': "Cette approche utilise un réseau expert (VGG16) pour extraire des caractéristiques complexes. L'algorithme Isolation Forest, basé sur les arbres de décision, isole ensuite les images qui sont sémantiquement différentes des autres. L'intuition est que les outliers, étant différents, sont plus faciles à isoler dans l'arbre (nécessitent moins de divisions pour être seuls dans une feuille). Cette technique permet de trouver des textures ou des formes inhabituelles.",
